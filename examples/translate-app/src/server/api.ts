...
/**
 * Who is the current user + their current target language. Called by the webview
 * on boot with `Authorization: Bearer <sessionToken>`.
 */
api.get('/me', async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    language: store.getLanguage(claims.userId),
  });
});
