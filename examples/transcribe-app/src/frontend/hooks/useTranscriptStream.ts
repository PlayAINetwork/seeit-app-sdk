...
/** Attempts before we surface "offline" (we keep retrying regardless). */
const OFFLINE_AFTER = 4;

/**
 * Subscribe to the live transcript SSE stream with automatic, backing-off
 * reconnection. The server backfills the current session on every (re)connect,
 * so dropping and restoring the connection is seamless.
 */
export function useTranscriptStream(token: string | undefined) {
  ...
  const connect = useCallback(() => {
    ...
    es.onopen = () => {
      ...
      // `live`/`idle` gets refined once the session header arrives.
  ...
  useEffect(() => {
    ...
    // Reconnect from scratch when the token changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token, connect]);

  return { segments, session, status, reconnect };
}
