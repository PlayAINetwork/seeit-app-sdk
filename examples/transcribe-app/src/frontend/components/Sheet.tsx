...
export function Sheet({
  ...
  onClose: () => void;
  title?: string;
  /** Optional element on the left of the header (e.g. a back button). */
  leading?: ReactNode;
  children: ReactNode;
}) {
  return (
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            className="relative flex max-h-[82vh] w-full max-w-md flex-col overflow-hidden rounded-t-4xl border border-white/10 bg-zinc-950/90 pb-safe shadow-sheet backdrop-blur-2xl"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
