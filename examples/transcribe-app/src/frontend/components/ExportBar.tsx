...
import { downloadText, transcriptToText } from "../lib/format.js";

/**
 * Copy / Save / Share toolbar for the current session. Pure client-side: builds
 * a plain-text transcript from the lines already on screen.
 */
export function ExportBar({
  ...
  const text = () => transcriptToText(startedAt, lines.map((t) => ({ text: t })));
  const filename = () =>
    `seeit-transcript-${new Date(startedAt).toISOString().slice(0, 16).replace(/[:T]/g, "-")}.txt`;
  ...
  const share = async () => {
    const data = { title: "SeeIt Transcript", text: text() };
